...
var firstLoad = function(){
  //默认按照工号排序
  requestData.sortType = 'nursing_class';
  //默认每页显示10条
...
var intotable = function(requestData){ 
  ...
  $.post('../php/service_management.php',requestData,function(data){
    if(data!=0){
      //将json数据解析为js可读的数组形式
      ...
      $.each(json,function(index,value){ 
        ...
        if(!value.total){
             html += '<tr>';
             html += '<td>' + value.nursing_class + '</td>';
             html += '<td>' + value.detailInfo + '</td>';
             html += '<td data-id="' + value.nursing_class + '">';
...
var operate = function(){
  ...
  $('td i.operateBtn').click(function(e){
    e.preventDefault();
    var nursing_class = $(this).parent().parent().attr('nursing_class');
    window.location = 'staff_edit.html?nursing_class=' + nursing_class; 
  });
}
